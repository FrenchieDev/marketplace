...
describe("MarkeplaceV2", () => {
  ...
  before(async () => {
    ...
    marketplace = await upgrades.upgradeProxy(marketplace.address, MarketplaceV2);
  ...
  describe("ERC1155", () =>{
    ...
    it("Should fail to create sell order (not apporved)", async () => {
      await expectRevert(
        marketplace.connect(alice).sell1155(nft1155_1.address, 1, 10, Number(time.duration.days(10)), 100),
        'Not approved'
      );
    })
    ...
    it("Should create a sell order", async () => {
      ...
      await marketplace.connect(alice).sell1155(nft1155_1.address, 1, 10, Number(time.duration.days(10)), 100);
    ...
    it("Should create a sell order for DAI", async () => {
      ...
      await marketplace.connect(bob).sell1155(nft1155_2.address, 1, 10, Number(time.duration.days(10)), 100);
    ...
    it("Should buy an order with DAI", async () => {
      ...
      let prevDAIRec = await dai.balanceOf(recipient.address);
      ...
      let postDAIRec = await dai.balanceOf(recipient.address);
      expect(Number(postBalance)).to.gt(Number(prevBalance));
      expect(Number(postDAI)).to.gt(Number(prevDAI));
      expect(Number(postDAIRec)).to.gt(Number(prevDAIRec));
    })
    ...
    it("Should buy an order with LINK", async () => {
      ...
      let prevLINKRec = await link.balanceOf(recipient.address);
      await marketplace.connect(alice).buyWithERC20(3,2);
      let postBalance = await nft1155_2.balanceOf(alice.address, 1);
      let postLINK = await link.balanceOf(bob.address);
      let postLINKRec = await link.balanceOf(recipient.address);
      expect(Number(postBalance)).to.gt(Number(prevBalance));
      expect(Number(postLINK)).to.gt(Number(prevLINK));
      expect(Number(postLINKRec)).to.gt(Number(prevLINKRec));
    })
  ...
  describe("721", () => {
    ...
    it("Should have created NFT721", async () => {
      await nft721.connect(alice).mint(1);
      await nft721.connect(bob).mint(2);
  
      let ownerAlice = await nft721.ownerOf(1);
      let ownerBob = await nft721.ownerOf(2);
      
      expect(ownerAlice).to.equal(alice.address);
      expect(ownerBob).to.equal(bob.address);
    })

    it("Should fail to create sell order (not apporved)", async () => {
      await expectRevert(
        marketplace.connect(alice).sell721(nft721.address, 1, Number(time.duration.days(10)), 100),
        'Not approved'
      );
    })
  
    it("Should create a sell order", async () => {
      await nft721.connect(alice).setApprovalForAll(marketplace.address,true);
      await marketplace.connect(alice).sell721(nft721.address, 1, Number(time.duration.days(10)), 100);
      let order = await marketplace.orders(5);
      expect(order.seller).to.equal(alice.address);
      expect(order.token).to.equal(nft721.address);
      expect(Number(order.tokenId)).to.equal(1);
      expect(Number(order.amount)).to.equal(1);
      expect(Number(order.price)).to.equal(100);
  
      // let tx = await marketplace.connect(alice).sell721(nft721.address, 1, Number(time.duration.days(10)), 100);
      // expectEvent(tx, "Sell", {
    ...
    it("Should buy an order with ETH", async () => {
      let prevOwner = await nft721.ownerOf(1);
      let prevEth = await alice.getBalance();
      let prevEthRec = await recipient.getBalance();
      await marketplace.connect(bob).buyWithEth(5, {value: toWei("1")});
      let postOwner = await nft721.ownerOf(1);
      let postEth = await alice.getBalance();
      let postEthRec = await recipient.getBalance();
      expect(postOwner).to.not.equal(prevOwner);
      expect(postOwner).to.equal(bob.address);
      expect(Number(postEth)).to.gt(Number(prevEth));
      expect(Number(postEthRec)).to.gt(Number(prevEthRec));
    })
    ...
    it("Should not buy an order not active", async () => {
      await expectRevert(
        marketplace.connect(bob).buyWithEth(5,{value: toWei("1")}),
        'Order not active'
      );
    })
  
    it("Should create a sell order for DAI", async () => {
      await nft721.connect(bob).setApprovalForAll(marketplace.address,true);
      await marketplace.connect(bob).sell721(nft721.address, 2, Number(time.duration.days(10)), 100);
      let order = await marketplace.orders(6);
      expect(order.seller).to.equal(bob.address);
      expect(order.token).to.equal(nft721.address);
      expect(Number(order.tokenId)).to.equal(2);
      expect(Number(order.amount)).to.equal(1);
      expect(Number(order.price)).to.equal(100);
    })
  
    it("Should buy an order with DAI", async () => {
      let prevOwner = await nft721.ownerOf(2);
      let prevDAI = await dai.balanceOf(bob.address);
      let prevDAIRec = await dai.balanceOf(recipient.address);
      await marketplace.connect(alice).buyWithERC20(6,1);
      let postOwner = await nft721.ownerOf(2);
      let postDAI = await dai.balanceOf(bob.address);
      let postDAIRec = await dai.balanceOf(recipient.address);
      expect(postOwner).to.not.equal(prevOwner);
      expect(postOwner).to.equal(alice.address);
      expect(Number(postDAI)).to.gt(Number(prevDAI));
      expect(Number(postDAIRec)).to.gt(Number(prevDAIRec));
    })
  
    it("Should create a sell order for LINK", async () => {
      await marketplace.connect(bob).sell721(nft721.address, 1, Number(time.duration.days(10)), 100);
      let order = await marketplace.orders(7);
      expect(order.seller).to.equal(bob.address);
      expect(order.token).to.equal(nft721.address);
      expect(Number(order.tokenId)).to.equal(1);
      expect(Number(order.amount)).to.equal(1);
    ...
    it("Should buy an order with LINK", async () => {
      ...
      let prevLINK = await link.balanceOf(bob.address);
      let prevLINKRec = await link.balanceOf(recipient.address);
      await marketplace.connect(alice).buyWithERC20(7,2);
      let postOwner = await nft721.ownerOf(1);
      let postLINK = await link.balanceOf(bob.address);
      let postLINKRec = await link.balanceOf(recipient.address);
      expect(postOwner).to.not.equal(prevOwner);
