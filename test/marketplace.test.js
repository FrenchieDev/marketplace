...
describe("Markeplace", () => {
  ...
  before(async () => {
    ...
    marketplace = await upgrades.deployProxy(MarketplaceV1, [
      recipient.address,
      1,
    ]);
  ...
  it("Should get DAI and LINK balance", async () => {
    ...
    await dex
      .connect(alice)
      .makeSwap(LINK, amountToBuy2, { value: toWei("1") });
  ...
  it("Should fail to create sell order (not apporved)", async () => {
    await expectRevert(
      marketplace
        .connect(alice)
        .sell(nft1155_1.address, 1, 10, Number(time.duration.days(10)), 100),
      "Not approved"
    );
  });
  ...
  it("Should create a sell order for DAI", async () => {
    ...
    await marketplace
      .connect(bob)
      .sell(nft1155_2.address, 1, 10, Number(time.duration.days(10)), 100);
  ...
  it("Should buy an order with DAI", async () => {
    ...
    await dai.approve(marketplace.address, String(balanceToApprove), {
      from: alice.address,
    });
  ...
  it("Should buy an order with LINK", async () => {
    ...
    await link.approve(marketplace.address, String(balanceToApprove), {
      from: alice.address,
    });
    let prevBalance = await nft1155_2.balanceOf(alice.address, 1);
