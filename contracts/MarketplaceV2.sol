...
contract MarketplaceV2 is Initializable, OwnableUpgradeable{
  ...
  */
  ...
  /**
    @notice order for a ERC1155 token
  ...
  */
  ...
  /**
    @notice creates a sell order for a ERC721 token
    @param token token contract address
    @param tokenId token id
    @param deadline order time available
    @param price
  ...
  */
  function buyWithEth(uint256 _orderId) public payable {
    ...
    if(order.semiFungible){
      IERC1155(order.token).safeTransferFrom(order.seller, msg.sender, order.tokenId, order.amount, "");
    }else{
      IERC721(order.token).safeTransferFrom(order.seller, msg.sender, order.tokenId, "");
    }
